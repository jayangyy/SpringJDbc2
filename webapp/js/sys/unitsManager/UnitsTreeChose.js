...
//		this.title=this.title+" ID:"+this.tag;
		//sparta 10/06/30 17:19 增加根据id查找节点的Action
		this.action	= url?url:"UnitById";
		this.loaded = false;		
		this.level  = -1;
		this.isLast = false;
		this.opened = false;
	...
	//			alert(caption)
		...
		this.toString = function(tv)
		{
			...
			var eA				= document.createElement("a");
				...
				//sparta 10/6/30 17:53 增加"+ '?treeId=' + me.tag"字串，以执行Action时携带必要的参数(节点id和节点名称)，并将me.action?me.action修改为UnitById
				//eA.href			= "UnitById?treeId=" + me.tag + "&nodeName=" + me.caption ;
			...
			//sparta 将下面的onclick 释放了出来（原来是注释掉的。）
			    ...
			    //新增加下列一行，在单击时关闭本窗口，并将相关参数给父窗口。 sparta 11/5/4
			    eA.onclick = function(e){  
					opener.document.UserForm.deptName.value = me.caption;
					opener.document.UserForm.userDeptId.value = me.tag;
					window.close();
					return;
				}
...
//			return false;
		...
		//sparta 10/5/27 17:52 增加下面的方法，以支持treeView响应双击事件
