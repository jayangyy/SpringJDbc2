public class Four extends Root //×óz
{
	...
	public boolean begin()
	{
			...
			(gTable.myTable[(gTable.x-1)/2+1][1]==0)&&(gTable.myTable[(gTable.x-1)/2+1][2]==0))
		{
			x=(gTable.x-1)/2;
			y=0;
			gTable.myTable[x][y]=4;
			gTable.myTable[x][y+1]=4;
			gTable.myTable[x+1][y+1]=4;
			gTable.myTable[x+1][y+2]=4;
			return true;
		}
		else
		{
		...
		switch(direct)
		{
                	case 1:if(y<=gTable.y-4)
			       {
					...
					store[1]=gTable.myTable[x+1][y+3];
						...
						gTable.myTable[x][y+2]=4;
						gTable.myTable[x+1][y+3]=4;
						y+=1;
						return true;
			      		 }
			      		 else
			...
			case 1:if(x<=gTable.x-4)
				{
			       		...
			       		store[2]=gTable.myTable[x+3][y];
			       		store[3]=gTable.myTable[x+2][y+2];
					store[4]=gTable.myTable[x+3][y+2];
					store[5]=gTable.myTable[x+2][y+3];
			       		if(isGo(6))
			       		{
						gTable.myTable[x][y]=0;
						gTable.myTable[x][y+1]=0;
			...
			case 2:if(true)
				{
					...
					store[4]=gTable.myTable[x][y];
					store[5]=gTable.myTable[x-1][y];
			       		if(isGo(6))
			      		 {
						gTable.myTable[x+1][y]=0;
						gTable.myTable[x+2][y]=0;
